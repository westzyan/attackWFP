...
def get_new_data(data, length_list, percent, single_instance):
    # 按照长度的百分比取样，每个类别取前single_instance个样本，最大长度按照百分比来
    new_data = []
    max_length = int(10000 * percent * 0.01)
    for i in range(95):
        # 原始的1000个样本
        ...
        for j in range(single_instance):
            ...
            length = int(int(length_list[i][j]) * percent * 0.01)
...
def LoadDataNoDefCW(percent, single_instance):
    ...
    length_list = np.loadtxt(dataset_dir + "length.txt", delimiter=",")
    data = get_new_data(data, length_list, percent, single_instance)
...
def test_specified():
    instances_list = [50, 100, 150, 200, 300, 500, 700,1000]
    for percent in range(35, 45, 5):
        for single_instance in instances_list:
            random.seed(0)
            ...
            X_train, y_train, X_valid, y_valid, X_test, y_test = LoadDataNoDefCW(percent, single_instance)
            ...
            print(confmat)
            with open("./increase_instance_percent_new.txt", 'a') as f:
                f.write("percent:{},single instance:{} acc:{}\n".format(percent * 0.01, single_instance, score_test[1]))
                f.write(score)
                f.write(str(confmat.tolist()) + "\n")
            f.close()
