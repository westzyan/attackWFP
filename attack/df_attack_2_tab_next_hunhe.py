...
def LoadDataNoDefCW(second):
    ...
    dataset_dir = "/home/thinkst/zyan/next_hunhe/"
    # X represents a sequence of traffic directions
    # y represents a sequence of corresponding label (website's label)
    # data = np.loadtxt(dataset_dir + "df_{}*95*300.txt".format(second), delimiter=",")
    data = pd.read_csv(dataset_dir + "df_{}*95*300.txt".format(second), delimiter=",", header=None)
    data = data.values
    print(data)
...
if __name__ == '__main__':
    ...
    for name in name_list:
        ...
        X_train, y_train, X_valid, y_valid, X_test, y_test = LoadDataNoDefCW(name)
        ...
        print(confmat)
        with open("./2_tab_next_mul_second.txt", 'a') as f:
            f.write("second:{} acc:{}\n".format(name, score_test[1]))
            f.write(score)
            f.write(str(confmat) + "\n")
        f.close()
