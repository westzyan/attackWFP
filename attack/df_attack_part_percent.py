...
def get_percent_data(data, percent):
    ...
    for i in range(len(data)):
        try:
            length = data[i].tolist().index(0) + 1
        except:
            length = 5000
        length = int(length * percent)
        item = data[i]
        new_item = [0] * int(5000 * percent)
...
def test():
    for i in range(5, 100, 5):
        ...
        # LENGTH = 100  # Packet sequence length
        ...
        X_train = get_percent_data(X_train, percent)
        X_train = X_train.astype('float32')
        X_valid = get_percent_data(X_valid, percent)
        X_valid = X_valid.astype('float32')
        X_test = get_percent_data(X_test, percent)
        ...
        with open("./percent_result.txt", 'a') as f:
            f.write("percent:{} acc:{}\n".format(percent, score_test[1]))
        f.close()
...
if __name__ == '__main__':
    ...
    #
    # # Convert data as float32 type
    # X_train = X_train.astype('float32')[:, 0:LENGTH]
    # X_valid = X_valid.astype('float32')[:, 0:LENGTH]
    # X_test = X_test.astype('float32')[:, 0:LENGTH]
    # y_train = y_train.astype('float32')
    # y_valid = y_valid.astype('float32')
    # y_test = y_test.astype('float32')
    #
